...
---
--- Replaces opencode.nvim's built-in vim.ui.select permission prompt with a
--- floating window that has selectable buttons. The window does NOT steal focus —
--- you keep typing in your buffer. When ready, C-w w into it, use h/l to choose,
--- Enter to confirm, Esc/q to hide, <leader>op to toggle visibility.
---
--- Edit permissions still show the normal diff UI (da/dr keymaps).
---
--- Integration lives in lua/exp/opencode.lua (monkey-patches the request handler).
...
function M.handle(event, server)
